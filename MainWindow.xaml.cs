...
namespace FolderSelectorApp
{
    ...
    public static class FileSizeFormatter
    {
        ...
        public static string FormatSize(long bytes)
        {
            ...
            int digitGroups = (int)Math.Log(bytes, 1024);
            digitGroups = Math.Min(digitGroups, SizeUnits.Length - 1);
            double size = bytes / Math.Pow(1024, digitGroups);
            return $"{size:F2} {SizeUnits[digitGroups]}";
        }
    }
        ...
        private static readonly Dictionary<string, HashSet<string>> CategoryExtensions = new Dictionary<string, HashSet<string>>
    {
        ...
        public static string GetFileCategory(string extension)
        {
            ...
            foreach (var category in CategoryExtensions)
            {
    ...
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private void ProcessFile(FileInfo file, string indent, bool isLast)
        {
            ...
            var category = FileAnalyzer.GetFileCategory(ext);
            var attributes = FileAnalyzer.GetFileAttributes(file);
        ...
        private void AppendScanSummary()
        {
            var summary = new StringBuilder()
                ...
                .AppendLine($"Общий размер: {FileSizeFormatter.FormatSize(_currentStats.TotalSize)}")
        ...
        private async Task SaveToFileAsync(object _)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    {
                        await writer.WriteAsync(DirectoryContent);
                    }
        ...
        private void ShowMessage(string message, string caption, MessageBoxImage icon) =>
            Application.Current.Dispatcher.InvokeAsync(() => MessageBox.Show(message, caption, MessageBoxButton.OK, icon));
